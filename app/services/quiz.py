...
# ------------------------------------------------------------------
# Absolute path to the Lambda entry file
quiz_lambda_handler = load_lambda_handler(
    lambda_file_path=QUIZ_LAMBDA_PATH,
    module_name="quiz_lambda_module",
)
...
def _call_lambda(path: str, query: Dict[str, Any], body: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    result = quiz_lambda_handler(event, context=None)
...
def get_quiz(patientId: str) -> Dict[str, Any]:
    """Mirror `/quiz` – Get te Quiz details"""
    return _call_lambda(
        path="/quiz",
