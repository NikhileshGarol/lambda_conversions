from db.db_connection import Session, SleepReadingsDetails, SleepReadings
from db.UserDet import fetch_patient_details
import pandas as pd
from datetime import datetime, timedelta

def fetch_sleep_readings_for_specific_day(mobile_number, specific_date=None, interval=None):
    """
    Fetch sleep readings for a specific day or a range of days based on the interval.
    ...
    Returns:
        tuple: A DataFrame of sleep readings and an error message (if any).
    """
    ...
    with Session() as session:
        try:
            # Base query to fetch sleep readings
            query = (
                session.query(SleepReadingsDetails)
                .join(SleepReadings, SleepReadingsDetails.sleep_readings_id == SleepReadings.id)
                .filter(SleepReadings.patient_id == user_id)
            )
            ...
            if interval:
                ...
                query = query.filter(SleepReadingsDetails.date.between(start_date, end_date))
            elif specific_date:  # Filter for a specific date if provided
                specific_date = pd.to_datetime(specific_date).date()
                query = query.filter(SleepReadingsDetails.date == specific_date)

            # Fetch the results and convert them to a DataFrame
            readings = query.order_by(SleepReadingsDetails.date).all()
            if readings:
                df = pd.DataFrame([
                    {
                        'date': r.date,
                ...
                # Ensure the 'value' and 'level' columns are numeric
                df['value'] = pd.to_numeric(df['value'], errors='coerce')
                df['level'] = pd.to_numeric(df['level'], errors='coerce')

                return df, None  # Return the DataFrame and no error
            else:
                print(f"No Sleep readings found for patient_id: {user_id}")
                return pd.DataFrame(), None  # Return an empty DataFrame if no readings found

        except Exception as e:
            print(f"Error while fetching Sleep readings for patient_id {user_id}: {e}")
