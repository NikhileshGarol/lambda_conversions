...
def fetch_heart_rate_readings_for_specific_day(mobile_number, specific_date=None, interval=None):
    """
    Fetch heart rate readings for a specific day or a range of days based on the interval.
    ...
    Returns:
        tuple: A DataFrame of heart rate readings and an error message (if any).
    """
    ...
    with Session() as session:
        try:
            # Base query to fetch heart rate readings
            query = (
            ...
            if interval:
                ...
                query = query.filter(HeartRateReadings.timestamp.between(start_date, end_date))
            elif specific_date:  # Filter for a specific date if provided
                specific_date = pd.to_datetime(specific_date).date()
                query = query.filter(HeartRateReadings.timestamp.between(specific_date, specific_date + timedelta(days=1)))

            # Fetch the results and convert them to a DataFrame
            readings = query.order_by(HeartRateReadings.timestamp).all()
            if readings:
                df = pd.DataFrame([
