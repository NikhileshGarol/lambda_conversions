from db.db_connection import Session, StressReadings, BleSummary
from db.UserDet import fetch_patient_details
import pandas as pd
from datetime import datetime, timedelta

def fetch_stress_readings_for_specific_day(mobile_number, specific_date=None, interval=None):
    """
    Fetch Stress readings for a specific day or a range of days based on the interval.
    ...
    Returns:
        tuple: A DataFrame of Stress readings and an error message (if any).
    """
    ...
    with Session() as session:
        try:
            # Base query to fetch Stress readings
            query = (
                session.query(StressReadings)
                .join(BleSummary, StressReadings.ble_summary_id == BleSummary.id)
            ...
            if interval:
                ...
                query = query.filter(StressReadings.timestamp.between(start_date, end_date))
            elif specific_date:  # Filter for a specific date if provided
                specific_date = pd.to_datetime(specific_date).date()
                query = query.filter(StressReadings.timestamp.cast('date') == specific_date)

            # Fetch the results and convert them to a DataFrame
            readings = query.order_by(StressReadings.timestamp).all()
            if readings:
                df = pd.DataFrame([
                ...
                print(f"No Stress readings found for patient_id: {user_id}")
                return pd.DataFrame(), None  # Return an empty DataFrame if no readings found

        except Exception as e:
            print(f"Error while fetching Stress readings for patient_id {user_id}: {e}")
