from db.db_connection import Session, ActivityReadings
from db.UserDet import fetch_patient_details
import pandas as pd
from datetime import datetime, timedelta

def fetch_activity_readings_for_specific_day(mobile_number, specific_date=None, interval=None):
    """
    Fetch activity readings for a specific day or a range of days based on the interval.
    ...
    Returns:
        tuple: A DataFrame of activity readings and an error message (if any).
    """
    ...
    with Session() as session:
        try:
            # Base query to fetch Activity readings
            query = session.query(ActivityReadings).filter(ActivityReadings.patient_id == user_id)
            ...
            if interval:
                ...
                query = query.filter(ActivityReadings.date.between(start_date, end_date))
            elif specific_date:  # Filter for a specific date if provided
                specific_date = pd.to_datetime(specific_date).date()
                query = query.filter(ActivityReadings.date == specific_date)
            
            # Fetch the results and convert them to a DataFrame
            readings = query.order_by(ActivityReadings.date).all()
            if readings:
                df = pd.DataFrame([
                    {
                        'date': r.date,
                        'activity_type': r.activity_type,
                        'total_exercise_duration': r.total_exercise_duration,
                        'total_calories_burned': r.total_calories_burned,
                        'total_distance': r.total_distance,
                        'total_step': r.total_step
                    } 
                    for r in readings
                ])
                
                # Ensure numeric fields are properly converted
                numeric_columns = ['total_exercise_duration', 'total_calories_burned', 'total_distance', 'total_step']
                for col in numeric_columns:
                    df[col] = pd.to_numeric(df[col], errors='coerce')

                return df, None  # Return the DataFrame and no error
            else:
                print(f"No activity readings found for patient_id: {user_id}")
                return pd.DataFrame(), None  # Return an empty DataFrame if no readings found

        except Exception as e:
            print(f"Error while fetching activity readings for patient_id {user_id}: {e}")
