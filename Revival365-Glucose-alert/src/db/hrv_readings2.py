from db.db_connection import Session, HRVReadings, BleSummary
from db.UserDet import fetch_patient_details
import pandas as pd
from datetime import datetime, timedelta

def fetch_hrv_readings_for_specific_day(mobile_number, specific_date=None, interval=1):
    """
    Fetch HRV readings for a specific day or a range of days based on the interval.
    ...
    Returns:
        tuple: A DataFrame of HRV readings and an error message (if any).
    """
    ...
    with Session() as session:
        try:
            # Base query to fetch HRV readings
            query = (
                session.query(HRVReadings)
                .join(BleSummary, HRVReadings.ble_summary_id == BleSummary.id)
            ...
            if interval:
                ...
                query = query.filter(HRVReadings.timestamp.between(start_date, end_date))
            elif specific_date:  # Filter for a specific date if provided
                specific_date = pd.to_datetime(specific_date).date()
                query = query.filter(HRVReadings.timestamp.between(specific_date, specific_date + timedelta(days=1)))

            # Fetch the results and convert them to a DataFrame
            readings = query.order_by(HRVReadings.timestamp).all()
            if readings:
                df = pd.DataFrame([
                ...
                print(f"No HRV readings found for patient_id: {user_id}")
                return pd.DataFrame(), None  # Return an empty DataFrame if no readings found

        except Exception as e:
            print(f"Error while fetching HRV readings for patient_id {user_id}: {e}")
