import json
from db.stress_readings2 import fetch_stress_readings_for_specific_day
from db.UserDet import fetch_patient_details

def get_stress_data_as_json(mobile_number, specific_date=None,interval=None):
    # Fetch stress readings for the given mobile number and specific date
    df, error = fetch_stress_readings_for_specific_day(mobile_number, specific_date,interval)
    ...
    # Construct the JSON output with stress readings and patient details
    data_json = {
        "stress_readings": df[['timestamp', 'value']].to_dict(orient='records'),
...
if __name__ == "__main__":
    ...
    response = get_stress_data_as_json(mobile_number, specific_date)
